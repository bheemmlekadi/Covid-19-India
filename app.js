...
app.get("/districts/:districtId/", async(request,response)=> {
    ...
    response.send(
        convertDistrictDbObjectToResponseObject(district)
    );
});
...
// API 8 Returns an object containing the state name
...
app.get("/districts/:districtId/details/", async(request,response)=> {
    ...
    WHERE 
    district_id = ${districtId};`;
    const state = await database.get(getStateNameBasedOnDistrictIdQuery);
